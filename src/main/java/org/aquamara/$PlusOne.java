package org.aquamara;

// Plus One
// 30/8/2023
// https://leetcode.com/explore/learn/card/array-and-string/201/introduction-to-array/1148/
public class $PlusOne {

    public static void main(String[] args) {
        int[] ints02 = plusOne(new int[]{9,9,9});
        int[] ints01 = plusOne(new int[]{4,3,9,9});
        int[] ints0 = plusOne(new int[]{1,2,3});
        int[] ints1 = plusOne(new int[]{4,3,2,1});
        int[] ints2 = plusOne(new int[]{9});
        System.out.println();
    }

    public static int[] plusOne(int[] digits) {
        int length = digits.length;

        for (int i = length - 1; i >= 0; i--) {
            if (digits[i] < 9) {
                ++digits[i];
                return digits;
            } else {
                digits[i] = 0;
            }
        }

        int[] digits2 = new int[length + 1];
        digits2[0] = 1;

        return digits2;
    }

    public static int[] plusOne1(int[] digits) {
        int length = digits.length;
        int plus = 1;
        for (int i = length - 1; i >= 0; i--) {
            digits[i] = (digits[i] + plus) % 10;
            if (plus != 0 && digits[i] != 0)
                plus = 0;
        }
        if (plus == 1) {
            int[] digits2 = new int[length + 1];
            digits2[0] = 1;
            return digits2;
        }
        return digits;
    }

    public static int[] plusOne2(int[] digits) {
        int length = digits.length;
        int plus = 1;
        int memory = 0;
        for (int i = length - 1; i >= 0; i--) {
            memory = digits[i] + plus;
            digits[i] = (digits[i] + plus) % 10;
            if (plus != 0 && memory <= 9)
                plus = 0;
        }
        if (plus == 1) {
            int[] digits2 = new int[length + 1];
            digits2[0] = 1;
            return digits2;
        }
        return digits;
    }
}

/*
You are given a large integer represented as an integer array digits, where each digits[i] is the ith digit
of the integer. The digits are ordered from most significant to least significant in left-to-right order.
The large integer does not contain any leading 0's.
Increment the large integer by one and return the resulting array of digits.

Example 1:
Input: digits = [1,2,3]
Output: [1,2,4]
Explanation: The array represents the integer 123.
Incrementing by one gives 123 + 1 = 124.
Thus, the result should be [1,2,4].

Example 2:
Input: digits = [4,3,2,1]
Output: [4,3,2,2]
Explanation: The array represents the integer 4321.
Incrementing by one gives 4321 + 1 = 4322.
Thus, the result should be [4,3,2,2].

Example 3:
Input: digits = [9]
Output: [1,0]
Explanation: The array represents the integer 9.
Incrementing by one gives 9 + 1 = 10.
Thus, the result should be [1,0].

Constraints:
1 <= digits.length <= 100
0 <= digits[i] <= 9
digits does not contain any leading 0's.
 */
